...
from head_touch_detector import extract_events
...
def main():
    ...
    signal_rows = load_signal(args.signal_csv)
    ...
    for threshold in thresholds:
        for enter_frames in enter_options:
            for exit_frames in exit_options:
                ...
                results.append((f1, -mae, precision, recall, tp, fp, fn, threshold, enter_frames, exit_frames))

    results.sort(key=lambda r: (r[0], r[1]), reverse=True)

    print(f"{'F1':>5} {'prec':>5} {'rec':>5} {'TP':>3} {'FP':>4} {'FN':>3} {'MAE':>6}  "
          f"{'thresh':>6} {'enter':>5} {'exit':>4}")
    for f1, neg_mae, precision, recall, tp, fp, fn, threshold, enter_frames, exit_frames in results[:20]:
        mae = -neg_mae
        mae_str = f"{mae:.2f}" if mae != float("inf") else "  n/a"
        print(f"{f1:5.2f} {precision:5.2f} {recall:5.2f} {tp:3d} {fp:4d} {fn:3d} {mae_str:>6}  "
              f"{threshold:6.2f} {enter_frames:5d} {exit_frames:4d}")
